...
def pseudo_chroot_setup() -> None:
    nix_eval_process = subprocess.run([ 'nix', ] + nixExperimentalFlags + [ 'eval', '.#nixosConfigurations.' + installer_variables['hostname'] + '._module.specialArgs.nixosSystemConfig.coreConfig.systemUser.username', ], stdout=subprocess.PIPE, text=True)
