...
ci_variables["supported_system_dep_attr_names"] = [
    "isoImages",
    "homeConfigurations",
    "devShells",
    "packages",
...
def get_all_supported_systems() -> None:
    for exclusive_nix_system in ci_variables["supported_systems"]:
        if "--exclusive-nix-system-{}".format(exclusive_nix_system) in sys.argv:
            exclusive_nix_system_check_command = [
                "nix",
                "run",
                ".#miscPackages.{}.binfmtCheck".format(exclusive_nix_system),
            ]
            print("DEBUG: Verifying buildability for `{}`".format(exclusive_nix_system_check_command))
            exclusive_nix_system_check_process = subprocess.run(
                exclusive_nix_system_check_command, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True
            )
    ...
    if "--use-emulation" in sys.argv:
        for nix_system in ci_variables["supported_systems"]:
            if nix_system == native_system:
                continue
            emulation_check_command = [
                "nix",
                "run",
                ".#miscPackages.{}.binfmtCheck".format(nix_system),
            ]
            print("DEBUG: Checking emulation for `{}`".format(emulation_check_command))
            emulation_check_process = subprocess.run(
                emulation_check_command, stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True
            )
...
def get_supported_nixos_systems() -> None:
    nix_eval_command = [
        ...
        ".#nixosConfigurations",
        "--apply",
        "configs: builtins.mapAttrs (name: value: value.config.nixpkgs.hostPlatform.system) configs",
    ]
    ...
    for hostname in nixosSystems:
        if nixosSystems[hostname] in ci_variables["all_supported_systems"]:
            ci_variables["nixosConfigurations"].append(
                ".#nixosConfigurations.{}.config.customOptions.finalBuildTarget".format(hostname)
            )
    return
...
async def main():
    ...
    ci_variables["nix_build_targets"] = (
        ci_variables["nixosConfigurations"]
        + ci_variables["homeConfigurations"]
        + ci_variables["packages"]
        + ci_variables["devShells"]
    ...
    if len(ci_variables["nix_build_targets"]) > 0:
        if "--evaluate-outPaths" in sys.argv:
                ...
                if nix_build_target in ci_variables["outPaths"]:
                    ...
                    if "--github-ci-shortcut" in sys.argv:
                        ...
                        if nix_path_info_process.returncode == 0:
                            if nix_path_info_process.stdout.startswith("No such path."):
                                ci_variables["ci_errors"].append(
                                    "ERROR: `{}` has not been built yet".format(eval_out_path)
                                )
                                ci_variables["late_exit_code"] = 1
                        else:
                            ci_variables["ci_errors"].append(
                                "ERROR: couldn't `curl` path for `{}` ".format(eval_out_path)
                            )
                            ci_variables["late_exit_code"] = 1

                else:
                    print(
                        "WARN: Nix build target `{}` probably cannot be built for some reason, please check.".format(
                            nix_build_target
                        )
                    )
                    if "--github-ci-shortcut" in sys.argv:
            ...
            build_all_targets_process = subprocess.run(
                make_nix_build_command(ci_variables["nix_build_targets"]),
                stdout=sys.stdout,
                stderr=sys.stderr,
                text=True,
                check=False,
            ...
            if build_all_targets_process.returncode != 0:
                print(
                    "ERROR: One or more Nix build targets could not be built. Building each one by one to figure out the fault one."
                )
