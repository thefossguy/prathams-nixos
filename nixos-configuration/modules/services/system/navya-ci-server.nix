...
lib.mkIf config.customOptions.localCaching.servesNixDerivations {
  systemd.services."${serviceConfig.unitName}" = {
    ...
    preStart = "[[ -f /etc/nixos/flake.lock ]] && touch /etc/nixos/flake.lock";
    script = ''
      ${pkgs.navya-ci}/bin/navya-ci \
          ...
          ${
            ...
            ) "--nix-copy-machine 's3://nix-cache-thefossguy?endpoint=638311974dc10256cb5e899da03d1935.r2.cloudflarestorage.com'"
          } \
          ${
            lib.strings.optionalString (
              config.networking.hostName == "chaturvyas"
