...
in
{
  ...
  services = {
    ...
    openssh = {
      ...
      hostKeys = lib.lists.optionals (config.customOptions.persistence.enable or false) [
        {
          path = "/persistent/state/etc/ssh/host-keys/ssh_host_ed25519_key";
          type = "ed25519";
        }
        {
          path = "/persistent/state/etc/ssh/host-keys/ssh_host_rsa_key";
          type = "rsa";
          bits = 8192;
        }
      ];
