...
lib.mkIf (config.customOptions.luksDevice.UUID != null) {
  ...
  boot.initrd.systemd.services."luks-unlock" = {
    ...
    script = ''
      ...
      ykman otp calculate 1 ${config.customOptions.luksDevice.challengeStringHex} | \
          cryptsetup open ${cryptsetupArgs} --key-file - ${config.customOptions.luksDevice.blockDevice} ${config.customOptions.luksDevice.label}
    '';
  };
}
